...
*Stand-alone fork of [ehough/docker-nfs-server](https://github.com/ehough/docker-nfs-server). [N0rthernL1ghts/docker-nfs-server](https://github.com/N0rthernL1ghts/docker-nfs-server)*
## Why?
...
Starting the `ghcr.io/bspwr/nfs-server` image will launch an NFS server. You'll need to supply some information upon container startup, which we'll cover below, but briefly speaking your `docker run` command might look something like this:
...
1. **Use `--cap-add SYS_ADMIN` or `--privileged`**
   ...
   As noted in the [requirements](#requirements), the container will need additional privileges. So your `run` command will need *either*:
       ...
       docker run --cap-add SYS_ADMIN ... ghcr.io/bspwr/nfs-server
       
    or

       docker run --privileged ... ghcr.io/bspwr/nfs-server
...
1. **Expose the server ports**
   ...
   * If your clients connect via **NFSv4 only**, you can get by with just TCP port `2049`:
         ...
         docker run -p 2049:2049 ... ghcr.io/bspwr/nfs-server
